...
void addlog(char *desc)
{
	...
	for (register int i = LOGSIZE; i >= 0; i--) 
		if (loga[i][0] != '\0') 
			strncpy(loga[i+1], loga[i], sizeof(loga[i+1])-1);
	_snprintf(loga[0], sizeof(loga[0]), "[%.2d-%.2d-%4d %.2d:%.2d:%.2d] %s", st.wMonth, st.wDay, st.wYear, st.wHour, st.wMinute, st.wSecond, desc);
...
void showlog(SOCKET sock, char *chan, BOOL notice, BOOL silent, char *filter)
{
	...
	for (int i = 0, j = 0; i < LOGSIZE && j < entries; i++, j++) 
		if (loga[i][0] != '\0') {
			if (!filter || tmp != 0)
				irc_privmsg(sock, chan, loga[i], notice, TRUE);
			else if (lstrstr(loga[i], filter))
				irc_privmsg(sock, chan, loga[i], notice, TRUE);
		}
...
{
	for (register int i = 0;i < LOGSIZE; loga[i++][0] = '\0');
	...
	for (int i = 0, j = 0; i < LOGSIZE && j < entries; i++, j++) 
		if (loga[i][0] != '\0') {
			if (showlog.filter[0] == '\0' || tmp != 0)
				irc_privmsg(showlog.sock, showlog.chan, loga[i], showlog.notice, TRUE);
			else if (lstrstr(loga[i], showlog.filter))
				irc_privmsg(showlog.sock, showlog.chan, loga[i], showlog.notice, TRUE);
		}
